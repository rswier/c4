...
long expr(long lev)
{
  ...
  if (!tk) { printf("%d: unexpected eof in expression", line); exit(-1); }
    ...
    *++e = IMM; *++e = ival; next();
    while (tk == '"') next();
    data = (char *)((long)data + 4 & -4); ty = PTR;
  }
  else if (tk == Id) {
    d = id; next();
    if (tk == '(') {
...
{
  long i, fd, t, bt, *d, cycle, poolsz, *idmain, *pc, *sp, *bp, a;
  ...
  p = "char else enum if long return while "
